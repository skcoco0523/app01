...
export default class Player extends Phaser.GameObjects.Container {
    ...
    constructor(scene, x, y, characterKey, scale, hitboxW, hitboxH, footY) {
        ...
        // 🌟【ここを追加！】このキャラの画像だけ、拡大縮小時のガタガタを消して滑らかにする設定を強制適用
        if (scene.textures.exists(`${this.characterKey}_atlas`)) {
            scene.textures.get(`${this.characterKey}_atlas`).setFilter(Phaser.Textures.FilterMode.LINEAR);
        }

        // 🌟 変更後：固有の物理設定を適用する新メソッド（後述）に処理を丸投げする
        if (!this.motionData) {
            console.error(`[Player] motionData not found for key: ${this.characterKey}_motion`);
        } else {
            this.updatePhysicsByForm();
        }
    ...
    play(key, ignoreIfPlaying = false) {
        ...
        if (key.startsWith('right_')) {
            ...
            this._flipX = true;
        } else if (key === 'front' || key.startsWith('front_')) {
            this.currentForm = 'front';
            this._flipX = false;
        }

        // 🌟 変更後：向きの切り替えが確定したタイミングで、物理パラメータをリアルタイム更新する
        this.updatePhysicsByForm();
    ...
    // 🌟 新設：現在の向き（form）に合わせてヒットボックスとスペックをマージ・更新するメソッド
    updatePhysicsByForm() {
        const physics = this.motionData.physics || {};
        const defaultConfig = physics.default || {};
        const formConfig = physics[this.currentForm] || {};

        // 各パラメーターについて、個別設定(form)があれば優先、なければ共通(default)を適用
        this.hitboxW = formConfig.hitboxWidth ?? defaultConfig.hitboxWidth ?? 118;
        this.hitboxH = formConfig.hitboxHeight ?? defaultConfig.hitboxHeight ?? 326;
        this.footY = formConfig.footY ?? defaultConfig.footY ?? 90;
        this.hitboxOffsetX = formConfig.offsetX ?? defaultConfig.offsetX ?? 0;
        this.globalPartScale = formConfig.globalPartScale ?? defaultConfig.globalPartScale ?? 0.8;

        // 速度やジャンプ力などのスペック数値もJSONからマージ
        this.walkSpeed = formConfig.walkSpeed ?? defaultConfig.walkSpeed ?? 260;
        this.jumpVelocity = formConfig.jumpVelocity ?? defaultConfig.jumpVelocity ?? -660;
        this.diveVelocity = formConfig.diveVelocity ?? defaultConfig.diveVelocity ?? 1200;
    ...
    jump(enableDoubleJump, thunderColor) {
        ...
        if (this.body.touching.down) {
            this.setVelocityY(this.jumpVelocity); Synth.jump(); this.jumpCount = 1; // 🌟 変更
        } else if (enableDoubleJump && this.jumpCount < 2) {
            this.body.velocity.y = 0; 
            this.setVelocityY(this.jumpVelocity * 0.94); // 🌟 変更 (2段目の比率を維持)
            Synth.jump(); this.jumpCount = 2;
            Environment.createExplosion(this.scene, this.x, this.y + 20, thunderColor, 8, 150);
        }
    }

    startDive() {
        this.scene.isDiving = true; this.jumpCount = 99; 
        this.setVelocity(0, this.diveVelocity); // 🌟 変更
    ...
    handleMovement(isInterruptRunning, currentAnimKey) {
        ...
        // 🌟 変更後：攻撃中（_attack）であっても、速度を0に固定する処理をまるごと消去！
