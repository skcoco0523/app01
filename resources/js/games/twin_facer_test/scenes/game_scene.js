...
export default class GameScene extends Phaser.Scene {
    ...
    init() {
        ...
        this.SETTINGS = {
            ...
            // 🌟 変更後：直書きされていたマジックナンバーをここに集約
            boundsLeft: 19,          // 画面左端の壁制限
            boundsRight: 431,        // 画面右端の壁制限
            comboGracePeriod: 2500,  // コンボが途切れる時間(ms)
            attackCooldown: 350      // 攻撃の連射クールダウン(ms)
        };
        ...
        this.currentSide = 'right'; this.isDiving = false; // 🌟 廃止された isFrontMode を削除
    ...
    create() {
        ...
        // 🌟 修正：physics.add.group ではなく、ただの「this.add.group()」に変更する
        // 弾自身が最初から物理ボディを持っているので、普通のグループに入れるだけで、速度リセットや重力バグを完全に回避できます！
        this.arrows = this.add.group();

        this.cameras.main.scrollY = Math.round(this.autoScrollY);
        ...
        // コントローラーの接続（全画面モードに仕様変更されたためオプションは不要）
        InputController.init(this);
        this.setupControllerEvents();

        // 🌟 修正：正面モードを廃止。移動フラグをOFFにし、最初の向き（右向き）の待機ポーズで開始する
        InputController.moveRight = false;
        InputController.moveLeft = false;
        this.player.play(`${this.currentSide}_idle`);
    ...
    setupControllerEvents() {
        ...
        InputController.on('onAnyInput', () => { 
            // 🌟 修正：ここでは時間を記録するだけに留め、勝手にダッシュを開始させない
        ...
        InputController.on('onSwipeDown', () => { 
            if (!this.player.body.touching.down && !this.isDiving && this.player.jumpCount < 2) {
        ...
        // 🌟 修正：左右にスワイプ（攻撃）されたタイミングで、停止状態からでも確実にオートランを起動・再開させる
        InputController.on('onSwipeRight', () => { 
            this.setForm('right'); 
            InputController.moveRight = true;  // 右ダッシュを強制ON
            InputController.moveLeft = false;
            this.attack('right'); 
        });
        InputController.on('onSwipeLeft', () => { 
            this.setForm('left'); 
            InputController.moveRight = false;
            InputController.moveLeft = true;   // 左ダッシュを強制ON
            this.attack('left'); 
        });
    }
    ...
    setupOverlapRules() {
        ...
        // ゴール到達
        this.physics.add.overlap(this.player, this.goalPortal, () => { if (this.gameState === 'PLAYING') this.triggerStageClear(); });
    }

    update(time, delta) {
        if (this.gameState !== 'PLAYING' || !this.player || !this.player.body) return; 

        // 🌟 変更後：集約した定数（SETTINGS）を使用するように変更
        if (time - this.lastComboTime > this.SETTINGS.comboGracePeriod) this.uiComboText.setVisible(false);
        ...
        // 🌟 変更後：集約した定数（SETTINGS）を使用するように変更
        if (this.player.x < this.SETTINGS.boundsLeft) { this.player.x = this.SETTINGS.boundsLeft; this.player.setVelocityX(0); }
        if (this.player.x > this.SETTINGS.boundsRight) { this.player.x = this.SETTINGS.boundsRight; this.player.setVelocityX(0); } // 🌟 修正：末尾に } を追加して閉じる
        ...
        // 🌟 修正：オートラン化に伴い、放置で正面を向いて立ち止まる判定を完全に無効化
        ...
        if (!this.isDiving && !isInterruptRunning) {
            ...
            // 🌟 修正：正面表示を完全廃止し、常に現在の向き（currentSide）に基づいたアニメーションを行う
            if (!this.player.body.touching.down) {
                this.player.play(vy < 0 ? `${this.currentSide}_jump` : `${this.currentSide}_fall`, true);
            } else if (vx !== 0) {
                this.player.play(`${this.currentSide}_walk`, true);
            } else {
                this.player.play(`${this.currentSide}_idle`, true);
            }
    ...
    toggleDirection() { 
        // まずは通常通りキャラクターの向き（フォーム）を反転
        this.setForm(this.currentSide === 'right' ? 'left' : 'right'); 
        
        // 🌟 修正：下スワイプによる方向転換の時は、オートランフラグを消して物理速度を即座に0（停止）にする
        InputController.moveRight = false;
        InputController.moveLeft = false;
    ...
    setForm(targetSide) {
        ...
        // ：フォームが切り替わったら、自動移動の方向フラグも即座に反転させる
        InputController.moveRight = isRight;
        InputController.moveLeft = !isRight;
    }

    attack(side) {
        if (this.time.now < this.lastAttackTime + this.SETTINGS.attackCooldown || this.isDiving) return;
        ...
        if (side === 'right') {
            ...
            // 🌟 変更後：新しい Arrow クラスを new してグループに追加するだけ！
            // 引数の最後には「向き（左に飛ばすので -1）」を引き渡します
            const arrow = new Arrow(this, this.player.x - 20, this.player.y, -1);
            this.arrows.add(arrow);

            // 🌟 追加：もし右向きの攻撃だったら、矢の向きも右に反転させる
            //const magic = new MagicBall(this, this.player.x - 20, this.player.y, 1);
            //this.arrows.add(magic);
        }
    }
    ...
    handlePlayerLanding() {
        if (this.isDiving) {
            ...
            // ：下降攻撃の衝撃で着地したら、即座に物理速度を0にし、オートランフラグもリセットして停止させる
            InputController.moveRight = false;
            InputController.moveLeft = false;
            this.player.setVelocityX(0);
        }
    ...
    updateCombo() {
        ...
        if (this.combo > 1) { 
            // 🌟 修正：連続ヒット時に古いアニメーションが残っているとバグるため、一度消去する
            this.tweens.killTweensOf(this.uiComboText);
            
            this.uiComboText.setText(`${this.combo} COMBO!`).setVisible(true).setScale(1.5); 
            
            // 🌟 修正：より安定した標準の tween 呼び出しに変更
            this.tweens.add({ 
                targets: this.uiComboText, 
                scale: 1, 
                duration: 150, 
                ease: 'Back.easeOut' 
            }); 
    ...
    setupUI() {
        // 🌟 修正：WebGLでの黒箱バグを防ぐため、文字の周囲に十分な透明の余白（パディング）を定義
        const uiPadding = { left: 15, right: 15, top: 15, bottom: 15 };

        this.uiModeText = this.add.text(225, 45, 'RIGHT FORM', { 
            fontFamily: 'Orbitron', fontSize: '22px', fill: '#ffd700', fontWeight: 'bold', padding: uiPadding
        }).setOrigin(0.5).setShadow(0,0,'#ffd700',10).setScrollFactor(0);

        this.uiScoreText = this.add.text(30, 40, 'SCORE: 0', { 
            fontFamily: 'Orbitron', fontSize: '13px', fill: '#fff', fontWeight: 'bold', padding: uiPadding
        }).setScrollFactor(0);

        this.uiHpText = this.add.text(330, 40, 'LIFE: ' + '❤️'.repeat(this.hp), { 
            fontSize: '13px', padding: uiPadding
        }).setScrollFactor(0);

        this.uiComboText = this.add.text(225, 150, '0 COMBO!', { 
            fontFamily: 'Orbitron', fontSize: '26px', fill: '#ff00ff', fontWeight: 'bold', padding: uiPadding
        }).setOrigin(0.5).setVisible(false).setScrollFactor(0);

        this.uiTimeText = this.add.text(225, 78, 'TIME: ' + this.gameTime + 's', { 
            fontFamily: 'Orbitron', fontSize: '13px', fill: '#00ffff', padding: uiPadding
